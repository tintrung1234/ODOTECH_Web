            ...
            <div className="flex flex-col gap-3 sm:gap-4">
                <div className="flex-1 relative">
                    ...
                    <input
                        ...
                        className="w-full h-9 sm:h-10 pl-10 pr-4 bg-gray-50 border border-gray-200 rounded-lg text-xs sm:text-sm focus:bg-white focus:border-teal-500 focus:ring-2 focus:ring-teal-100 outline-none transition-all"
                    />
                </div>
                <div className="flex flex-col sm:flex-row bg-gray-50 rounded-lg p-2 sm:p-1 border border-gray-200 gap-2">
                    <div className="flex items-center gap-2 px-2">
                        <Filter size={14} className="text-gray-400 shrink-0" />
                        <select
                            value={filterCategory}
                            onChange={(e) => onCategoryChange(e.target.value as '' | ExpenseCategory)}
                            className="flex-1 h-8 bg-transparent border-none text-xs sm:text-sm text-gray-700 outline-none focus:ring-0 cursor-pointer"
                        >
